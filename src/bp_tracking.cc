...
namespace ORB_SLAM2 {
    ...
    cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp) {
        ...
        mCurrentFrame = new Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary,
    ...
    void Tracking::StereoInitialization() {
        if (mCurrentFrame->N > 500) {
            ...
            for (int i = 0; i < mCurrentFrame->N; i++) {
                ...
                if (z > 0 && z < mThDepth * 2) {                            // Depth constraint
...
//            return;
        ...
        for (int i = 0; i < mLastFrame->N; ++i) {
            ...
            if (bCreateNew) {
                float z = mLastFrame->mvDepth[i];
                if (z < 0 || z > mThDepth * 2) continue;                // Depth constraint
                cv::Mat X3D = mLastFrame->UnprojectStereo(i);
                MapPoint *pNewMP = new MapPoint(X3D, mpMap, mLastFrame, i);
...
//            if (pMP)
//            {
//                LOG(INFO) << "MapPoint: " << pMP->mnId << ", Frame: " << pMP->mnFirstFrame << " type: " << pMP->mType << " ,found: " << pMP->GetFoundByFrame();
//            }
//        }
